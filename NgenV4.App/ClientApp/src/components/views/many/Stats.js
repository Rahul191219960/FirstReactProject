...
export default class Stats extends React.Component {
    ...
    render(){
            ...
            <div key={k.field.id} className="f-stats">
                ...
                <div className="stat-values">
                    <div className="fl-stats">
                        {k.sum ? itemAggr(k.id+'sum', i18n.i18n_stats.total, k.sum) : null}
                        {k.avg ? itemAggr(k.id+'avg', i18n.i18n_stats.avg, formattedValue(format.decimalString(k.avg), k.field)) : null}
                        {k.min===k.max ? (
                            itemAggr(k.id+'min-max', i18n.i18n_stats.min+' = '+i18n.i18n_stats.max, formattedValue(k.min, k.field))
                        ) : (
                            k.avg ? (
                                <div className="field-range">
                                    {itemAggr(k.id+'min', i18n.i18n_stats.min, formattedValue(k.min, k.field))}
                                    <div>
                                        <Range min={k.min} max={k.max} avg={k.avg} ></Range>
                                    </div>
                                    {itemAggr(k.id+'max', i18n.i18n_stats.max, formattedValue(k.max, k.field))}
                                </div>
                            ) : (
                                <React.Fragment>
                                    {itemAggr(k.id+'min', i18n.i18n_stats.min, k.min)}
                                    {itemAggr(k.id+'max', i18n.i18n_stats.max, k.max)}
                                </React.Fragment>
                            )
                        )}
                    </div>
        ...
        if(this.state.loading){
            ...
            body = <Alert type="info" title={i18n.i18n_stats.noData} message={i18n.i18n_stats.emptyData}/> 
        }else if(this.state.error){
            body = <Alert title="Error" message={this.state.error.message}/> 
        }else{
            const recCount = no01n(data.count, model.name, model.namePlural)
            const commentsCount = no01n(data.nb_comments, i18n.i18n_comments.comment, i18n.i18n_comments.comments)
            body = (
                <div className="evol-stats">
                    <div className="cols-2">
                        <div className="col-1">
                            ...
                            { wTimestamp ? (
                                <label className="evo-label">{data.u_date_week_count || 'No'} {i18n.i18n_stats.weekUpdates}</label>
                            ) : null }
                        </div>
                        <div className="col-2">
                            { wTimestamp ? (
                                <div>
                                    <div className="stat-field">
                                        <span>{i18n.i18n_stats.lastUpdate}:</span> 
                                        {data.u_date_max}
                                    </div> 
                                    <div className="stat-field">
                                        <span>{i18n.i18n_stats.firstInsert}:</span> 
                                        {data.c_date_min}
                                    </div> 
                                </div>
                            ) : null }
