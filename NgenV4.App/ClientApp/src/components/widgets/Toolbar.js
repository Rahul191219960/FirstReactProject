...
//import {i18n.i18n_actions, i18n.i18n_msg} from '../../i18n/i18n'
...
const menuItems = {
    new: { id: 'edit/0', label: i18n.i18n_actions.new, icon: 'add', n: 'x', readonly: false },
    del: { id: 'del', label: i18n.i18n_actions.delete1, icon: 'delete', n: '1', readonly: false },
    //filter: {id:'filter', label: i18n.i18n_actions.filter, icon:'filter', n:'n'},
    filter: { id: 'filter', label: i18n.i18n_actions.filter, icon: 'filter', n: 'x' },
    export: { id: 'export', label: i18n.i18n_actions.export1, icon: 'export', n: 'x' },
    save: { id: 'save', label: i18n.i18n_actions.save, icon: 'save', n: '1', readonly: false },
    //import: {id:'import', label: i18n.i18n_actions.bImport, icon:'import',n:'x'},
    ...
    //sel: {id: 'selections', label: i18n.i18n_actions.Selections, icon:'star', n:'x'},
    views: {
        browse: { id: 'browse', label: i18n.i18n_actions.browse, icon: 'browse', n: '1' },// // ReadOnly
        edit: { id: 'edit', label: i18n.i18n_actions.edit, icon: 'edit', n: '1', readonly: false },// // All Fields for editing

        list: { id: 'list', label: i18n.i18n_actions.list, icon: 'list', n: 'n' },
        cards: { id: 'cards', label: i18n.i18n_actions.cards, icon: 'cards', n: 'n' },
        //scatter: {id:'scatter', label: i18n.i18n_actions.bScatter, icon:'certificate',n:'n'},
...
class Toolbar extends React.Component {
    ...
    render() {
        ...
        function buttonLink(menu, idOrFun, urlQuery = '') {
            return isFunction(idOrFun) ? (
                <span key={idx++} onClick={idOrFun} className="fakeLink"><Icon name={menu.icon} tooltip={menu.label} theme="light"></Icon> </span>
            ) : (
                    <Link key={idx++} to={ep + menu.id + '/' + idOrFun + urlQuery}><Icon name={menu.icon} tooltip={menu.label} theme="light"></Icon> </Link>
                )
        }
        ...
        if (id) {
            ...
            if (view !== 'charts' && view !== 'stats') {
                actions.push(buttonLink(menuItems.filter, this.filterGrid, q));
                actions.push(buttonLink(menuItems.export, this.exportMany, q));
            }
        }

        const showFilterContainer = this.state.showFilter ? "" : "hidden";
                    ...
                    <div>
                        <div className="modal-content">
                            <div className="modal-header">
                                ...
                                <h4 className="modal-title">{i18n.i18n_msg.delete.replace('{0}', m.name)}</h4>
                            </div>
                            <div className="modal-body">
                                {i18n.i18n_msg.deleteConfirmation.replace('{0}', m.name)}
                            </div>
                            <div className="modal-footer">
                                <button key="bDelCancel" onClick={this.closeModal} className="btn btn-default" data-dismiss="modal">{i18n.i18n_actions.cancel}</button>
                                <button key="bDelOK" onClick={this.deleteOne} className="btn btn-primary" data-dismiss="modal">{i18n.i18n_actions.ok}</button>
                            </div>
                        </div>
                    </div>
                </Modal>
        ...
        if (entity && id && m) {
            axios.delete(apiPath + entity + '/' + id)
                .then(response => {
                    ...
                    toast.success(i18n.i18n_actions.deleted.replace('{0}', capitalize(m.name)), {
    ...
    */
    ...
    filterGrid = () => {
        this.props.filterContainerStateHandler();
    }
}
