...
namespace NgenV4.Service
{
    public class UserGroupService : BaseService, IUserGroupService
    {
        ...
        public UserGroupService(IMapper imapper, IUnitOfWork iUnitOfWork)
        {
            _mapper = imapper;
        ...
        public async Task DeleteRecord(string SNo)
        {
            try
            {
                bool isDeleted = await _unitOfWork.IUserGroupRepository.DeleteRecord(SNo);
                _unitOfWork.Commit();
                this.ErrorNo = _unitOfWork.IUserGroupRepository.ErrorNo;
                this.ErrorMessage = _unitOfWork.IUserGroupRepository.ErrorMessage;
            ...
            {
                this.ErrorNo = 80001;
                this.ErrorMessage = "UserGroupService:DeleteRecord() - " + this.GetGlobalMessage("Error");
                this.ErrorLog("UserGroupService:DeleteRecord() - " + ex.ToString());

            }
        }
        public async Task<IEnumerable<UserGroupViewModelGrid>> GetList(DataSourceRequestViewModel request)
        {
            try
            {
                ...
                var result = await _unitOfWork.IUserGroupRepository.GetList(map);
                var contact = _mapper.Map<IEnumerable<UserGroupViewModelGrid>>(result);
                return contact;
        ...
        public async Task<UserGroupViewModel> GetRecord(string SNo)
        {
            try
            {
                var result = await _unitOfWork.IUserGroupRepository.GetRecord(SNo);
                var UserGroup = _mapper.Map<UserGroupViewModel>(result);
                return UserGroup;
            }
            catch (System.Exception ex)
            {
                this.ErrorNo = 80001;
                this.ErrorMessage = "UserGroupService:GetRecord() - " + this.GetGlobalMessage("Error");
                this.ErrorLog("UserGroupService:GetRecord() - " + ex.ToString());
                return null;
            }
        }
        public async Task SaveRecord(UserGroupViewModel ar)
        {
            try
            {
                UserGroup UserGroup = _mapper.Map<UserGroupViewModel, UserGroup>(ar);
                this.ErrorNo = _unitOfWork.IUserGroupRepository.ErrorNo;
                this.ErrorMessage = _unitOfWork.IUserGroupRepository.ErrorMessage;
                if (this.ErrorNo == 0)
                {
                    await _unitOfWork.IUserGroupRepository.SaveRecord(UserGroup);
                    _unitOfWork.Commit();
                    if (this.ErrorNo == 0)
            ...
            {
                this.ErrorNo = 80001;
                this.ErrorMessage = "UserGroupService:SaveRecord() - " + this.GetGlobalMessage("Error");
                this.ErrorLog("UserGroupService:SaveRecord() - " + ex.ToString());
            }
        }
        public async Task UpdateRecord(JObject ar, int id)
        {
            try
            {
                ...
                await _unitOfWork.IUserGroupRepository.UpdateRecord(ar, id);
                _unitOfWork.Commit();
                this.ErrorNo = _unitOfWork.IUserGroupRepository.ErrorNo;
            ...
            {
                this.ErrorNo = 80001;
                this.ErrorMessage = "UserGroupService:UpdateRecord() - " + this.GetGlobalMessage("Error");
                this.ErrorLog("UserGroupService:UpdateRecord() - " + ex.ToString());
            }
        }
    }
}
